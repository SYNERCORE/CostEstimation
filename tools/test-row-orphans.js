...
ck('by default nothing is dropped', /let _rowOrphans=0,_rowDrop=false,_rowMissing=0;/.test(db));
ck('a partial CE is never cached over the complete local copy', /if\(!_partial&&!_missing\)try\{await cePut/.test(db));
