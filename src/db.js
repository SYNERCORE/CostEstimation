...
const _SP_OPTIONAL_COLS = ['shicShares','shicTaskId','shicPax','shicOTHours','shicPerDiem','shicInfo','shicTier','shicHours','shicKW','shicRunHrs'];
...
async function dbCacheAllCEs(progressCb){
  ...
  const resRows=await _spGetTolerant(spList('CE_Resources'),null,'Id,shicCEId,shicTab,shicDesc,shicQty,shicUOM,shicCost,shicDays,shicTaskId,shicShares,shicTier,shicHours,shicKW,shicRunHrs');
...
async function dbLoadCE(id){
  ...
  try{const[hR,mR,rR]=await Promise.all([_spGetTolerant(spList('CEs'),`Id eq ${id}`,'Id,Title,shicType,shicClient,shicDesc,shicTotal,shicSavedBy,shicSavedAt,shicScope,shicNotes,shicApprovers,shicMob,shicDemob,shicMisc,shicSOW,shicInfo'),_spGetByCE(spList('CE_MP'),id,'Id,shicRole,shicRate,shicShift,shicDays,shicQty,shicPax,shicOTHours,shicPerDiem,shicTaskId,shicShares'),_spGetByCE(spList('CE_Resources'),id,'Id,shicTab,shicDesc,shicQty,shicUOM,shicCost,shicDays,shicTaskId,shicShares,shicTier,shicHours,shicKW,shicRunHrs')]);if(!hR.length)return null;const h=hR[0];const _ce=_assembleCE(h,mR,rR);
